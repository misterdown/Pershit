...
namespace render {
    namespace RenderVulkanUtils {
        ...
        static const char* deviceExtensions[] = { VK_KHR_SWAPCHAIN_EXTENSION_NAME, "VK_EXT_extended_dynamic_state" };
...
render_manager& render_manager::execute() {
    ...
    VkFence fence;
    vkCreateFence(logicalDevice, &fenceInfo, 0, &fence);
    ...
    VkSemaphore sema;
    vkCreateSemaphore(logicalDevice,&semaInfo, 0, &sema );

    uint32_t imageIndex;
    vkAcquireNextImageKHR(logicalDevice, swapchain, UINT64_MAX, sema, fence, &imageIndex);

    VkPipelineStageFlags waitforlove = VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT;
    ...
    const VkSubmitInfo submit {
        ...
        .pWaitDstStageMask = &waitforlove,
        .commandBufferCount = 1,
        .pCommandBuffers = &commandBuffers[imageIndex],
        .signalSemaphoreCount = 0,
        .pSignalSemaphores = 0,
    };
    vkQueueSubmit(logicalDevice.graphicsQueue, 1, &submit, 0);
    ...
    vkDestroySemaphore(logicalDevice, sema, 0);
